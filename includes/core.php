...
function picker_suggest_lookup() {
    ...
    foreach ( $posts as $post ):
        ...
        // Prepare label
        $post_label = picker_trunc_words( $post->post_title, 11 ) . ( ( $post->post_status != 'publish' ) ? ' (' . $post->post_status . ')' : '' );
        if ( $post->post_type != PKR_POST_TYPES ) $post_label = $post_label . ' (' . $post->post_type . ')';

        // Create suggestion item object
        $suggestion = array();
        $suggestion[ 'label' ] = $post_label;
